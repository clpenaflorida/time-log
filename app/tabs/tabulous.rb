Tabulous.setup do
  ...
  tabs do
    ...
    time_entries_tab do
      text          { 'Time Entries' }
      link_path     { time_entries_path }
      visible_when  { true }
      enabled_when  { true }
      active_when   { in_action('any').of_controller('time_entries') }
    end

    user_settings_tab do
      text          { 'User Settings' }
      link_path     { user_settings_path }
      visible_when  { true }
      enabled_when  { true }
      active_when   { in_action('any').of_controller('user_settings') }
    end
